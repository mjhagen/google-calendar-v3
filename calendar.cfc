component accessors=true {
  ...
  public component function init( ) {
    ...
    var calendarScope = variables.jl.create( "com.google.api.services.calendar.CalendarScopes" ).CALENDAR;
    var driveScope = variables.jl.create( "com.google.api.services.drive.DriveScopes" ).DRIVE;
    var collections = variables.jl.create( "java.util.Collections" ).singletonList( calendarScope & " " & driveScope );
    var credentialsBuilder = variables.jl.create( "com.google.api.client.googleapis.auth.oauth2.GoogleCredential$Builder" );
    var FSkeyFile = variables.jl.create( "java.io.File" ).init( root & "/credentials/" & keyFile );
    var httpTransport = variables.jl.create( "com.google.api.client.http.javanet.NetHttpTransport" ).init( );
    var jsonFactory = variables.jl.create( "com.google.api.client.json.gson.GsonFactory" ).init( );
    ...
    variables.calendarService = variables.jl.create( "com.google.api.services.calendar.Calendar$Builder" )
      .init( httpTransport, jsonFactory, credentials )
      .setApplicationName( variables.appName )
      .build( );

    variables.driveService = variables.jl.create( "com.google.api.services.drive.Drive$Builder" )
      .init( httpTransport, jsonFactory, credentials )
      .setApplicationName( variables.appName )
      .build( );
  ...
  public function getEvents( ) {
    ...
    if ( !isNull( variables.startDate ) ) {
      var tz = variables.jl.create( "java.util.TimeZone" ).getTimeZone( timeZone );
      var timeMin = variables.jl.create( 'com.google.api.client.util.DateTime' ).init( variables.startDate, tz );
